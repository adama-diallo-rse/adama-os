...
| **L1** Données                | Supabase UE, Drizzle, pgvector, RLS                  | **Livrée.** 8 migrations, de `0000_init.sql` a `0006_revirements.sql` plus le semis du registre produits. `ecosystem_products` (0001), `ecosystem_analytics` avec provenance et vue de compatibilité (0002). Dettes résorbées : `audit_requests` tranchée, enum `lead_source` aligné.                                                                 |
| **L2** Moteur                 | Calcul ESG lourd                                     | **Fermée. Aucun développement autorisé.**                                                                                                                                                                                                                                                                                                             |
| **L3** Intelligence           | RAG, adama.ai                                        | **Chaîne complète et durcie.** Un incident de retrieval fait échouer la réponse au lieu de générer sans source. `rag:verify` mesure un seuil de PERTINENCE, et non la simple présence d'une source : échec sous 0,45, avertissement entre 0,45 et 0,50. **Reste : déposer et ingérer le corpus.**                                                     |
...
| **L7** Acquisition            | OG dynamiques, UTM                                   | **Partielle.** Gabarit OG commun et onze images générées, convention UTM posée en un seul endroit. Calendrier éditorial et angles narratifs : travail de contenu, hors dépôt, P4.                                                                                                                                                                     |
...
| **L11** Tests                 | Filet minimal                                        | **Livrée.** Vitest, 35 fichiers de test. Au 2 septembre 2026 : 507 cas verts et 9 ignorés sur 516 sans base de recette, 514 verts et 2 ignorés avec le banc décrit dans `docs/RLS.md`. Les cas ignorés par défaut sont ceux qui exigent une base ou une clé de modèle. Pas de CI : le quota GitHub Actions est épuisé, les tests se lancent en local. |
| **L12** Continuité            | Sauvegarde, secrets, reprise                         | **Livrée.** Politique en une page, script d'export, inventaire des secrets, note de reprise. Le test de restauration a eu lieu le 2 septembre 2026 : 9 tables sur 9 restaurées à l'identique, comptes de lignes et empreintes md5 identiques, daté par le script dans `docs/restauration.json`. Reste à le refaire contre une copie de la vraie base. |
...
pnpm --filter @adama/web test         # 507 verts, 9 ignorés (2 septembre 2026)
